...
impl Program {
    ...
    pub fn set_float_1f(&self, name: &str, value:f32){
        unsafe {
    ...
    pub fn set_mat4f(&self, name: &str, value: &glm::Mat4){
        unsafe {
            let location = gl::GetUniformLocation(self.id, CString::new(name).unwrap().as_ptr());
            gl::UniformMatrix4fv(
                location,
                1,
                gl::FALSE,
                value.as_array().as_ptr() as *const gl::types::GLfloat
            ); 
        }
    }
}
